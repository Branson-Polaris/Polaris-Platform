/* vxbFdtMicrelPhy.c - driver for Micrel PHY chips */
...
--------------------
04dec20,x_z  probe more times to fix HW issue.
22feb19,gqn  ping lost issue fix (V7NET-2317)
15jan19,tcw  fix the compilation warnings of this file (F11377)
23jan18,yal  Fix EIOCGMEDIALIST returning Error issue (V7PRO-4170)
12dec17,yal  Correct PHY 1000M speed check bit (V7PRO-4146)
...
03aug15,m_w  add fdtMicrelPhyShutdown.
...
#endif /* _WRS_KERNEL */
#include <vxbMiiLib.h>
#include "vxbFdtMicrelPhy.h"

/* defines */

/* debug macro */

#undef MICREL_PHY_DEBUG
...
LOCAL STATUS fdtMicrelPhyModeSet (VXB_DEV_ID, UINT32);
LOCAL STATUS fdtMicrelPhyModeGet (VXB_DEV_ID, UINT32 *, UINT32 *);
LOCAL void   fdtMicrelPhyInit    (VXB_DEV_ID);
LOCAL STATUS fdtMicrelPhyProbe   (VXB_DEV_ID);
LOCAL STATUS fdtMicrelPhyAttach  (VXB_DEV_ID);
LOCAL STATUS fdtMicrelPhyDetach  (VXB_DEV_ID);
LOCAL STATUS fdtMicrelPhyShutdown(VXB_DEV_ID);
...
#ifdef MICREL_PHY_DEBUG
LOCAL void   fdtMicrelPhyInfoShow(VXB_DEV_ID);
#endif /* MICREL_PHY_DEBUG */

LOCAL VXB_DRV_METHOD fdtMicrelPhyMethods[] =
    {
    /* DEVICE API */
    { VXB_DEVMETHOD_CALL(vxbDevProbe),    fdtMicrelPhyProbe},
    { VXB_DEVMETHOD_CALL(vxbDevAttach),   fdtMicrelPhyAttach},
    { VXB_DEVMETHOD_CALL(vxbDevDetach),   fdtMicrelPhyDetach},
    { VXB_DEVMETHOD_CALL(vxbDevShutdown), fdtMicrelPhyShutdown},

    /* MII API */
    { VXB_DEVMETHOD_CALL(miiModeGet),     fdtMicrelPhyModeGet},
    { VXB_DEVMETHOD_CALL(miiModeSet),     fdtMicrelPhyModeSet},
    VXB_DEVMETHOD_END
    };
...
VXB_DRV  fdtMicrelPhyDrv =
    {
    { NULL } ,
    "Branson fdtMicrelPhy",            /* Name */
    "Branson Micrel PHY driver",    /* Description */
    VXB_BUSID_MII,          /* Class */
    VXB_DRVFLAG_PARAM,      /* Flags */
    0,                      /* Reference count */
    fdtMicrelPhyMethods,       /* Method table */
    micrelParams            /* Parameters */
    };

VXB_DRV_DEF(fdtMicrelPhyDrv)

/*******************************************************************************
*
* fdtMicrelPhyProbe - vxBus probe handler
*
...
LOCAL STATUS fdtMicrelPhyProbe
    (
    VXB_DEV_ID pDev
    )
    {
...
#ifdef MICREL_PHY_DEBUG
        fdtMicrelPhyInfoShow(pDev);
#endif

        miiOui = (UINT32)MII_OUI(pPhyDev->phyId1,pPhyDev->phyId2);
...
/*******************************************************************************
*
* fdtMicrelPhyShutdown - VxBus shutdown handler
*
...
LOCAL STATUS fdtMicrelPhyShutdown
    (
    VXB_DEV_ID  pDev
    )
    {
...
/*******************************************************************************
*
* fdtMicrelPhyDetach - VxBus detach handler
*
...
LOCAL STATUS fdtMicrelPhyDetach
    (
    VXB_DEV_ID pDev
    )
    {
...
/*******************************************************************************
*
* fdtMicrelPhyAttach - attach micreltric phy device
*
* This routine attach micreltric phy device with fdtMicrelPhyDrv,it creats device
...
LOCAL STATUS fdtMicrelPhyAttach
    (
    VXB_DEV_ID pDev
    )
    {
...
LOCAL void fdtMicrelPhyInit
    ...
    if ((miiSts & MII_SR_EXT_STS) != 0)
        ...
        (void)miiBusWrite (pDev, pDrvCtrl->miiPhyAddr, MII_MASSLA_CTRL_REG,
                           miiVal);
        }
    ...
    /*
     * Renesa Rzn1 embedded morthip 5 port switch uses the LED[0] from the PHYs
     * as a link up/down status signal, so program the PHYs to output this.
     */

    (void)miiBusRead (pDev, pDrvCtrl->miiPhyAddr, MICREL_PHYCON1_REG, &miiVal);
    miiVal |= MICREL_PHYCON1_LEDACT;
    (void)miiBusWrite (pDev, pDrvCtrl->miiPhyAddr, MICREL_PHYCON1_REG, miiVal);
...
/*******************************************************************************
*
* fdtMicrelPhyModeSet - set the PHY mode
*
...
LOCAL STATUS fdtMicrelPhyModeSet
    (
    VXB_DEV_ID pDev,
    UINT32 mode
    )
...
/*******************************************************************************
*
* fdtMicrelPhyModeGet - read the current PHY mode and link status
*
...
LOCAL STATUS fdtMicrelPhyModeGet
    (
    VXB_DEV_ID pDev,
    UINT32 * mode,
    UINT32 * status
    ...
    *status |= IFM_ACTIVE;
    ...
    if ((miiCtl & MII_CR_AUTO_EN) != 0)
        ...
        if (miiCtl & MII_CR_FDX)
            ...
            *mode |= IFM_HDX;
        if ((miiCtl & MII_CR_1000) == MII_CR_1000)
            *mode |= IFM_1000_T;
...
LOCAL void ksz9031PhyExtendedWrite
    ...
    (void)miiBusWrite(pDev, pDrvCtrl->miiPhyAddr,
                      KSZ9031RN_MMD_CTRL_REG, (UINT16)((UINT32)(mode << 14) | devAddr));
    (void)miiBusWrite(pDev, pDrvCtrl->miiPhyAddr,
...
/*********************************************************************
*
* fdtMicrelPhyInfoShow - show the micrel phy info for debug
*
...
LOCAL void fdtMicrelPhyInfoShow
    (
    VXB_DEV_ID pDev
    )
    {
