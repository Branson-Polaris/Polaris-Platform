...
void copyFile(char * supportFilePath)
{
	...
	else
	{
		...
		status = cp(supportFilePath , MMC_EMMC_DEV_NAME);
...
INT32 HRFSEMMCFormat(void)
{
	...
	fd = open(MMC_EMMC_DEV_NAME,0,0);
	/* Check for EMMC mount point */
	if(ERROR != fd)
	{
		EMMCdevProperty = devGetByName(MMC_EMMC_DEV_NAME);
		...
		else
		{
			...
			ret = hrfsFormat (MMC_EMMC_DEV_NAME, 0ll,0, 0);
			if(ERROR == ret)
			{
				printf("\nFormat error\n");
			}
...
{
	/* Comments from Tom: Recipes will only exist in the DB */
//	copyFile(UPDATED_WELD_RECIPE_PATH);		/* Copy UpdatedWeldRecipe.txt to eMMC */
	
	/* Comments from Tom: Default weld recipe should exist in code, not in a file */
...
/**************************************************************************//**
* \brief   -Copy database file into EMMC from ROMFS if file does not exist on EMMC.
* 			
* \param:
* pcSourceFilePath - Source File Path on ROMFS.
* pcDestFilePath - Destination File Path on EMMC.
* \return  - OK on Success else copy error code.
*
******************************************************************************/
int copyDBFileToEMMC(const char *pcSourceFilePath, const char *pcDestFilePath)
{
	int nStatus = OK;
	char szTempFilePath[100]="";
	
	//Check if the EMMC is hosting the database. 	
	if(access( pcDestFilePath, F_OK ) != 0)
	{
		printf("Database %s doesn't exist in EMMC.\n", pcDestFilePath);
		
		//Remove -wal and -shm DB files in case they were left undeleted in EMMC.
		strncpy(szTempFilePath, pcDestFilePath, sizeof(szTempFilePath) - 1);
		strncat(szTempFilePath,"-wal", 4);
		rm(szTempFilePath);
		strncpy(szTempFilePath, pcDestFilePath, sizeof(szTempFilePath) - 1);
		strncat(szTempFilePath,"-shm", 4);
		rm(szTempFilePath);
		
		//If there is no database in EMMC,copy it from ROMFS. 
		nStatus = cp(pcSourceFilePath, MMC_EMMC_DEV_NAME);
		if(nStatus != OK)
		{
			printf("Cannot copy database %s to EMMC. File copy error:%d\n", pcSourceFilePath, nStatus);
		}
	}
	return nStatus;
}
...
STATUS loadApplicationModule(INT8 typeOfApp)
{
	...
	if(KERNEL == typeOfApp)		/* Spawn Kernel space(DKM) application */
	{
		...
		fdLinkLayer = open (DKM_TEST_PATH, 0, 0);   	
		if (fdLinkLayer == ERROR) 
		{
			printErr("ERROR in Opening link layer - BransonDrv_DKM_Test.out\n");
			return ERROR;
		} 			
		...
		if( moduleId  != NULL) 
		{    	
			status = symFindByName(sysSymTbl, "start_test", (char **)&startfunction, &symtype);
			if ( status == 0 ) 
			{
				startfunction();
			}
			...
			{
				printErr("BransonDrv_DKM_Test Entry point not found error: %08X\n", errno); 
			} 
		}   
		else 
		{
			printErr("ERROR in loading BransonDrv_DKM_Test\n");
			return ERROR; 
		}		
	}
#endif
	...
	if(USER == typeOfApp) /* Spawn User space(RTP) application */
	{							
		...
		//Copy Database Files from ROMFS to EMMC if files do not exist in EMMC. 
		status |= copyDBFileToEMMC(COMMON_DB_FILE_PATH_ROMFS, COMMON_DB_FILE_PATH_EMMC);
		status |= copyDBFileToEMMC(P1_BASE_DB_FILE_PATH_ROMFS, P1_BASE_DB_FILE_PATH_EMMC);
