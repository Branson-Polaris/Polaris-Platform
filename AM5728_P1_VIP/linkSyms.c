/* linkSyms.c - dynamically generated configuration file */


/*
GENERATED: Wed Oct 13 13:35:25 +0800 2021
DO NOT EDIT - file is regenerated whenever the project changes
*/

typedef int (*FUNC) ();
extern int _Assert ();
extern int _Atexit ();
extern int _Besseli ();
extern int _Besseli0 ();
extern int _Besseli1 ();
extern int _Besseli_AS ();
extern int _Besseli_UAE ();
extern int _Besselj ();
extern int _Besselj0 ();
extern int _Besselj1 ();
extern int _Besselk ();
extern int _Besselk0 ();
extern int _Besselk1 ();
extern int _Bessely0 ();
extern int _Bessely1 ();
extern int _Cnd_do_broadcast_at_thread_exit ();
extern int _Dbl ();
extern int _Dclass ();
extern int _Deletegloballocale ();
extern int _Dsign ();
extern int _Exit ();
extern int _FBesseli0 ();
extern int _FBesseli1 ();
extern int _FBesseli_AS ();
extern int _FBesseli_UAE ();
extern int _FBesselj0 ();
extern int _FBesselj1 ();
extern int _FBesselk0 ();
extern int _FBesselk1 ();
extern int _FBessely0 ();
extern int _FBessely1 ();
extern int _FDclass ();
extern int _FDsign ();
extern int _FFpcomp ();
extern int _FRf_xp ();
extern int _FRiemann_approx ();
extern int _Fenv0 ();
extern int _Flt ();
extern int _Fltrounds ();
extern int _Fpcomp ();
extern int _Getctyptab ();
extern int _Getgloballocale ();
extern int _LBesseli0 ();
extern int _LBesseli1 ();
extern int _LBesseli_AS ();
extern int _LBesseli_UAE ();
extern int _LBesselj0 ();
extern int _LBesselj1 ();
extern int _LBesselk0 ();
extern int _LBesselk1 ();
extern int _LBessely0 ();
extern int _LBessely1 ();
extern int _LDclass ();
extern int _LDsign ();
extern int _LFpcomp ();
extern int _LRf_xp ();
extern int _LRiemann_approx ();
extern int _Ldbl ();
extern int _Lock_shared_ptr_spin_lock ();
extern int _Rf_xp ();
extern int _Riemann_approx ();
extern int _Stderr ();
extern int _Stdin ();
extern int _Stdout ();
extern int _Xmutex_cpp_unused ();
extern int _Xtime_diff_to_ts ();
extern int _ZN6Dinkum7codecvt10_Cvt_checkEmj ();
extern int _ZNSt10filesystem10_Close_dirEPv ();
extern int _ZNSt6_WinitC1Ev ();
extern int _ZNSt6locale6globalERKS_ ();
extern int _ZNSt6locale7_Locimp9_MakewlocERKSt8_LocinfoiPS0_PKS_ ();
extern int _ZNSt6locale7_Locimp9_MakexlocERKSt8_LocinfoiPS0_PKS_ ();
extern int _ZNSt8ios_base4InitC1Ev ();
extern int _ZNSt8ios_base7_AddstdEPS_ ();
extern int _ZNSt9exceptionD0Ev ();
extern int _ZSt10_Rng_abortPKc ();
extern int _ZSt11_Xbad_allocv ();
extern int _ZSt11setiosflagsNSt5_IosbIiE9_FmtflagsE ();
extern int _ZSt13_Execute_onceRSt9once_flagPFiPvS1_PS1_ES1_ ();
extern int _ZSt13_Syserror_mapi ();
extern int _ZSt14_Debug_messagePKcS0_j ();
extern int _ZSt14_Raise_handler ();
extern int _ZSt14_Random_devicev ();
extern int _ZSt14_Throw_C_errori ();
extern int _ZSt14set_unexpectedPFvvE ();
extern int _ZSt18uncaught_exceptionv ();
extern int _ZSt7_FiopenPKcNSt5_IosbIiE9_OpenmodeEi ();
extern int _ZSt7_MP_AddPyy ();
extern int _ZSt7nothrow ();
extern int _ZSt8_XLgammad ();
extern int _ZSt9terminatev ();
extern int _ZdaPv ();
extern int _ZdaPvRKSt9nothrow_t ();
extern int _ZdaPvS_ ();
extern int _ZdaPvj ();
extern int _ZdlPv ();
extern int _ZdlPvRKSt9nothrow_t ();
extern int _ZdlPvS_ ();
extern int _ZdlPvj ();
extern int _Znaj ();
extern int _ZnajPv ();
extern int _ZnajRKSt9nothrow_t ();
extern int _Znwj ();
extern int _ZnwjPv ();
extern int _ZnwjRKSt9nothrow_t ();
extern int __absvdi2 ();
extern int __absvsi2 ();
extern int __adddf3 ();
extern int __addsf3 ();
extern int __addvdi3 ();
extern int __addvsi3 ();
extern int __aeabi_cdcmpeq ();
extern int __aeabi_cdcmpeq_check_nan ();
extern int __aeabi_cdcmple ();
extern int __aeabi_cdrcmple ();
extern int __aeabi_cfcmpeq ();
extern int __aeabi_cfcmpeq_check_nan ();
extern int __aeabi_cfcmple ();
extern int __aeabi_cfrcmple ();
extern int __aeabi_d2f ();
extern int __aeabi_d2h ();
extern int __aeabi_d2iz ();
extern int __aeabi_d2lz ();
extern int __aeabi_d2uiz ();
extern int __aeabi_d2ulz ();
extern int __aeabi_dadd ();
extern int __aeabi_dcmpeq ();
extern int __aeabi_dcmpge ();
extern int __aeabi_dcmpgt ();
extern int __aeabi_dcmple ();
extern int __aeabi_dcmplt ();
extern int __aeabi_dcmpun ();
extern int __aeabi_ddiv ();
extern int __aeabi_dmul ();
extern int __aeabi_dneg ();
extern int __aeabi_dsub ();
extern int __aeabi_f2d ();
extern int __aeabi_f2h ();
extern int __aeabi_f2iz ();
extern int __aeabi_f2lz ();
extern int __aeabi_f2uiz ();
extern int __aeabi_f2ulz ();
extern int __aeabi_fadd ();
extern int __aeabi_fcmpeq ();
extern int __aeabi_fcmpge ();
extern int __aeabi_fcmpgt ();
extern int __aeabi_fcmple ();
extern int __aeabi_fcmplt ();
extern int __aeabi_fcmpun ();
extern int __aeabi_fdiv ();
extern int __aeabi_fmul ();
extern int __aeabi_fneg ();
extern int __aeabi_fsub ();
extern int __aeabi_h2f ();
extern int __aeabi_i2d ();
extern int __aeabi_i2f ();
extern int __aeabi_idiv ();
extern int __aeabi_idivmod ();
extern int __aeabi_l2d ();
extern int __aeabi_l2f ();
extern int __aeabi_lasr ();
extern int __aeabi_lcmp ();
extern int __aeabi_ldivmod ();
extern int __aeabi_llsl ();
extern int __aeabi_llsr ();
extern int __aeabi_lmul ();
extern int __aeabi_memclr ();
extern int __aeabi_memclr4 ();
extern int __aeabi_memclr8 ();
extern int __aeabi_memcmp ();
extern int __aeabi_memcmp4 ();
extern int __aeabi_memcmp8 ();
extern int __aeabi_memcpy ();
extern int __aeabi_memcpy4 ();
extern int __aeabi_memcpy8 ();
extern int __aeabi_memmove ();
extern int __aeabi_memmove4 ();
extern int __aeabi_memmove8 ();
extern int __aeabi_memset ();
extern int __aeabi_memset4 ();
extern int __aeabi_memset8 ();
extern int __aeabi_ui2d ();
extern int __aeabi_ui2f ();
extern int __aeabi_uidiv ();
extern int __aeabi_uidivmod ();
extern int __aeabi_ul2d ();
extern int __aeabi_ul2f ();
extern int __aeabi_ulcmp ();
extern int __aeabi_uldivmod ();
extern int __aeabi_unwind_cpp_pr0 ();
extern int __ashldi3 ();
extern int __ashrdi3 ();
extern int __clzdi2 ();
extern int __clzsi2 ();
extern int __cmpdf2 ();
extern int __cmpdi2 ();
extern int __cmpsf2 ();
extern int __ctzdi2 ();
extern int __ctzsi2 ();
extern int __cxa_bad_cast ();
extern int __cxa_call_unexpected ();
extern int __cxa_demangle ();
extern int __cxa_get_globals ();
extern int __cxa_guard_abort ();
extern int __cxa_pure_virtual ();
extern int __cxa_thread_atexit ();
extern int __cxa_uncaught_exception ();
extern int __cxa_vec_new ();
extern int __divdc3 ();
extern int __divdf3 ();
extern int __divdi3 ();
extern int __divmoddi4 ();
extern int __divmodsi4 ();
extern int __divsc3 ();
extern int __divsf3 ();
extern int __divsi3 ();
extern int __divtc3 ();
extern int __divxc3 ();
extern int __dynamic_cast ();
extern int __eprintf ();
extern int __eqdf2 ();
extern int __eqsf2 ();
extern int __errno ();
extern int __extendhfsf2 ();
extern int __extendsfdf2 ();
extern int __ffsdi2 ();
extern int __fixdfdi ();
extern int __fixdfsi ();
extern int __fixsfdi ();
extern int __fixsfsi ();
extern int __fixunsdfdi ();
extern int __fixunsdfsi ();
extern int __fixunssfdi ();
extern int __fixunssfsi ();
extern int __fixunsxfdi ();
extern int __fixunsxfsi ();
extern int __fixxfdi ();
extern int __floatdidf ();
extern int __floatdisf ();
extern int __floatdixf ();
extern int __floatsidf ();
extern int __floatsisf ();
extern int __floatundidf ();
extern int __floatundisf ();
extern int __floatundixf ();
extern int __floatunsidf ();
extern int __floatunsisf ();
extern int __gedf2 ();
extern int __gesf2 ();
extern int __gnu_f2h_ieee ();
extern int __gnu_h2f_ieee ();
extern int __gtdf2 ();
extern int __gtsf2 ();
extern int __ledf2 ();
extern int __lesf2 ();
extern int __lshrdi3 ();
extern int __ltdf2 ();
extern int __ltsf2 ();
extern int __moddi3 ();
extern int __modsi3 ();
extern int __muldc3 ();
extern int __muldf3 ();
extern int __muldi3 ();
extern int __mulodi4 ();
extern int __mulosi4 ();
extern int __mulsc3 ();
extern int __mulsf3 ();
extern int __multc3 ();
extern int __mulvdi3 ();
extern int __mulvsi3 ();
extern int __mulxc3 ();
extern int __nedf2 ();
extern int __negdf2 ();
extern int __negdi2 ();
extern int __negsf2 ();
extern int __negvdi2 ();
extern int __negvsi2 ();
extern int __nesf2 ();
extern int __paritydi2 ();
extern int __paritysi2 ();
extern int __popcountdi2 ();
extern int __popcountsi2 ();
extern int __powidf2 ();
extern int __powisf2 ();
extern int __powixf2 ();
extern int __subdf3 ();
extern int __subsf3 ();
extern int __subvdi3 ();
extern int __subvsi3 ();
extern int __truncdfhf2 ();
extern int __truncdfsf2 ();
extern int __truncsfhf2 ();
extern int __ucmpdi2 ();
extern int __udivdi3 ();
extern int __udivmoddi4 ();
extern int __udivmodsi4 ();
extern int __udivsi3 ();
extern int __umoddi3 ();
extern int __umodsi3 ();
extern int __unorddf2 ();
extern int __unordsf2 ();
extern int __vxatomic ();
extern int __wr_register_frame_info ();
extern int abort ();
extern int abort_message ();
extern int abs ();
extern int absSymbols_Common ();
extern int acos ();
extern int acosf ();
extern int acosh ();
extern int acoshf ();
extern int acoshl ();
extern int acosl ();
extern int aligned_alloc ();
extern int armGenTimerDrv ();
extern int asctime ();
extern int asctime_r ();
extern int asctime_s ();
extern int asin ();
extern int asinf ();
extern int asinh ();
extern int asinhf ();
extern int asinhl ();
extern int asinl ();
extern int assoc_laguerre ();
extern int assoc_laguerref ();
extern int assoc_laguerrel ();
extern int assoc_legendre ();
extern int assoc_legendref ();
extern int assoc_legendrel ();
extern int atan ();
extern int atan2 ();
extern int atan2f ();
extern int atan2l ();
extern int atanf ();
extern int atanh ();
extern int atanhf ();
extern int atanhl ();
extern int atanl ();
extern int atexit ();
extern int atof ();
extern int atoi ();
extern int atol ();
extern int atoll ();
extern int atomic_flag_clear ();
extern int atomic_flag_clear_explicit ();
extern int atomic_flag_test_and_set ();
extern int atomic_flag_test_and_set_explicit ();
extern int atomic_signal_fence ();
extern int atomic_thread_fence ();
extern int bcmp ();
extern int bcopy ();
extern int besseli0 ();
extern int besseli0f ();
extern int besseli0l ();
extern int besseli1 ();
extern int besseli1f ();
extern int besseli1l ();
extern int besselj0 ();
extern int besselj0f ();
extern int besselj0l ();
extern int besselj1 ();
extern int besselj1f ();
extern int besselj1l ();
extern int besselk0 ();
extern int besselk0f ();
extern int besselk0l ();
extern int besselk1 ();
extern int besselk1f ();
extern int besselk1l ();
extern int bessely0 ();
extern int bessely0f ();
extern int bessely0l ();
extern int bessely1 ();
extern int bessely1f ();
extern int bessely1l ();
extern int beta ();
extern int betaf ();
extern int betal ();
extern int bitFirstGet ();
extern int bsearch ();
extern int bsearch_s ();
extern int btowc ();
extern int bzero ();
extern int c16rtomb ();
extern int c32rtomb ();
extern int cabs ();
extern int cabsf ();
extern int cabsl ();
extern int cacos ();
extern int cacosf ();
extern int cacosh ();
extern int cacoshf ();
extern int cacoshl ();
extern int cacosl ();
extern int call_once ();
extern int calloc ();
extern int carg ();
extern int cargf ();
extern int cargl ();
extern int casin ();
extern int casinf ();
extern int casinh ();
extern int casinhf ();
extern int casinhl ();
extern int casinl ();
extern int catan ();
extern int catanf ();
extern int catanh ();
extern int catanhf ();
extern int catanhl ();
extern int catanl ();
extern int cbrt ();
extern int cbrtf ();
extern int cbrtl ();
extern int ccos ();
extern int ccosf ();
extern int ccosh ();
extern int ccoshf ();
extern int ccoshl ();
extern int ccosl ();
extern int ceil ();
extern int ceilf ();
extern int ceill ();
extern int cexp ();
extern int cexpf ();
extern int cexpl ();
extern int cfree ();
extern int checksum ();
extern int cimag ();
extern int cimagf ();
extern int cimagl ();
extern int clearerr ();
extern int clock ();
extern int clog ();
extern int clogf ();
extern int clogl ();
extern int cnd_broadcast ();
extern int cnd_destroy ();
extern int cnd_init ();
extern int cnd_signal ();
extern int cnd_timedwait ();
extern int cnd_wait ();
extern int comp_ellint_1 ();
extern int comp_ellint_1f ();
extern int comp_ellint_1l ();
extern int comp_ellint_2 ();
extern int comp_ellint_2f ();
extern int comp_ellint_2l ();
extern int comp_ellint_3 ();
extern int comp_ellint_3f ();
extern int comp_ellint_3l ();
extern int conj ();
extern int conjf ();
extern int conjl ();
extern int copysign ();
extern int copysignf ();
extern int copysignl ();
extern int cos ();
extern int cosf ();
extern int cosh ();
extern int coshf ();
extern int coshl ();
extern int cosl ();
extern int cpow ();
extern int cpowf ();
extern int cpowl ();
extern int cproj ();
extern int cprojf ();
extern int cprojl ();
extern int cpswPortDrv ();
extern int cpswSwCtrlDrv ();
extern int creal ();
extern int crealf ();
extern int creall ();
extern int csin ();
extern int csinf ();
extern int csinh ();
extern int csinhf ();
extern int csinhl ();
extern int csinl ();
extern int csqrt ();
extern int csqrtf ();
extern int csqrtl ();
extern int ctan ();
extern int ctanf ();
extern int ctanh ();
extern int ctanhf ();
extern int ctanhl ();
extern int ctanl ();
extern int ctime ();
extern int ctime_r ();
extern int ctime_s ();
extern int cyl_bessel_if ();
extern int cyl_bessel_il ();
extern int cyl_bessel_jf ();
extern int cyl_bessel_jl ();
extern int cyl_bessel_kf ();
extern int cyl_bessel_kl ();
extern int cyl_neumann ();
extern int cyl_neumannf ();
extern int cyl_neumannl ();
extern int difftime ();
extern int div ();
extern int dllCreate ();
extern int dllInit ();
extern int dosFsCacheCreate ();
extern int drand48 ();
extern int ellint_1 ();
extern int ellint_1f ();
extern int ellint_1l ();
extern int ellint_2 ();
extern int ellint_2f ();
extern int ellint_2l ();
extern int ellint_3 ();
extern int ellint_3f ();
extern int ellint_3l ();
extern int erand48 ();
extern int erf ();
extern int erfc ();
extern int erfcf ();
extern int erfcl ();
extern int erff ();
extern int erfl ();
extern int etherMultiAdd ();
extern int exit ();
extern int exp ();
extern int exp2 ();
extern int exp2f ();
extern int exp2l ();
extern int expf ();
extern int expint ();
extern int expintf ();
extern int expintl ();
extern int expl ();
extern int expm1 ();
extern int expm1f ();
extern int expm1l ();
extern int fabs ();
extern int fabsf ();
extern int fabsl ();
extern int fclose ();
extern int fdim ();
extern int fdimf ();
extern int fdiml ();
extern int fdopen ();
extern int fdtMicrelPhyDrv ();
extern int feclearexcept ();
extern int fegetenv ();
extern int fegetexceptflag ();
extern int fegetround ();
extern int feholdexcept ();
extern int feof ();
extern int feraiseexcept ();
extern int ferror ();
extern int fesetenv ();
extern int fesetexceptflag ();
extern int fesetround ();
extern int fetestexcept ();
extern int feupdateenv ();
extern int fflush ();
extern int fgetc ();
extern int fgetpos ();
extern int fgets ();
extern int fgetwc ();
extern int fgetws ();
extern int fileno ();
extern int flockfile ();
extern int floor ();
extern int floorf ();
extern int floorl ();
extern int fma ();
extern int fmaf ();
extern int fmal ();
extern int fmax ();
extern int fmaxf ();
extern int fmaxl ();
extern int fmin ();
extern int fminf ();
extern int fminl ();
extern int fmod ();
extern int fmodf ();
extern int fmodl ();
extern int fopen ();
extern int fopen_s ();
extern int fppProbe ();
extern int fprintf ();
extern int fprintf_s ();
extern int fputc ();
extern int fputs ();
extern int fputwc ();
extern int fputws ();
extern int fread ();
extern int free ();
extern int freopen ();
extern int freopen_s ();
extern int frexp ();
extern int frexpf ();
extern int frexpl ();
extern int fscanf ();
extern int fscanf_s ();
extern int fseek ();
extern int fseeko ();
extern int fsetpos ();
extern int ftell ();
extern int ftello ();
extern int ftpCommand ();
extern int ftrylockfile ();
extern int funlockfile ();
extern int fwide ();
extern int fwprintf ();
extern int fwprintf_s ();
extern int fwrite ();
extern int fwscanf ();
extern int fwscanf_s ();
extern int getaddrinfo ();
extern int getc ();
extern int getc_unlocked ();
extern int getchar ();
extern int getchar_unlocked ();
extern int getenv ();
extern int getenv_s ();
extern int gethostbyname ();
extern int getifaddrs ();
extern int getnameinfo ();
extern int getopt ();
extern int getoptInit ();
extern int gets ();
extern int gets_s ();
extern int getservbyname ();
extern int getservbyport ();
extern int getw ();
extern int getwc ();
extern int getwchar ();
extern int gmtime ();
extern int gmtime_r ();
extern int gmtime_s ();
extern int hashTblInit ();
extern int hermite ();
extern int hermitef ();
extern int hermitel ();
extern int hookAddToTail ();
extern int hookShow ();
extern int hypot ();
extern int hypotf ();
extern int hypotl ();
extern int ifconfig ();
extern int ilogb ();
extern int ilogbf ();
extern int ilogbl ();
extern int imaxabs ();
extern int imaxdiv ();
extern int index ();
extern int inet_aton ();
extern int inet_network ();
extern int ioHelp ();
extern int ipAttach ();
extern int ipcom_cfgfile_parse ();
extern int ipcom_register_dev_name_mapping ();
extern int ipnet_eth_add_hdr ();
extern int ipnet_ip4_input ();
extern int ipnet_ip4_mcast_input ();
extern int ipnet_ip6_input ();
extern int ipnet_ip6_mcast_input ();
extern int ipnet_sock_netlink_register ();
extern int ipnet_sock_route_register ();
extern int ipnet_vlan_if_init ();
extern int isalnum ();
extern int isalpha ();
extern int isblank ();
extern int iscntrl ();
extern int isdigit ();
extern int isgraph ();
extern int islower ();
extern int isprint ();
extern int ispunct ();
extern int isspace ();
extern int isupper ();
extern int iswalnum ();
extern int iswalpha ();
extern int iswblank ();
extern int iswcntrl ();
extern int iswctype ();
extern int iswdigit ();
extern int iswgraph ();
extern int iswlower ();
extern int iswprint ();
extern int iswpunct ();
extern int iswspace ();
extern int iswupper ();
extern int iswxdigit ();
extern int isxdigit ();
extern int jobAdd ();
extern int jrand48 ();
extern int labs ();
extern int laguerre ();
extern int laguerref ();
extern int laguerrel ();
extern int lcong48 ();
extern int ldexp ();
extern int ldexpf ();
extern int ldexpl ();
extern int ldiv ();
extern int legendre ();
extern int legendref ();
extern int legendrel ();
extern int lgamma ();
extern int lgammaf ();
extern int lgammal ();
extern int llabs ();
extern int lldiv ();
extern int llrint ();
extern int llrintf ();
extern int llrintl ();
extern int llround ();
extern int llroundf ();
extern int llroundl ();
extern int localeconv ();
extern int localtime ();
extern int localtime_r ();
extern int localtime_s ();
extern int log ();
extern int log10 ();
extern int log10f ();
extern int log10l ();
extern int log1p ();
extern int log1pf ();
extern int log1pl ();
extern int log2 ();
extern int log2f ();
extern int log2l ();
extern int logb ();
extern int logbf ();
extern int logbl ();
extern int logf ();
extern int logl ();
extern int longjmp ();
extern int lrand48 ();
extern int lrint ();
extern int lrintf ();
extern int lrintl ();
extern int lround ();
extern int lroundf ();
extern int lroundl ();
extern int lstInit ();
extern int m_prepend ();
extern int malloc ();
extern int mblen ();
extern int mbrlen ();
extern int mbrtoc16 ();
extern int mbrtoc32 ();
extern int mbrtowc ();
extern int mbsinit ();
extern int mbsrtowcs ();
extern int mbsrtowcs_s ();
extern int mbstowcs ();
extern int mbstowcs_s ();
extern int mbtowc ();
extern int memalign ();
extern int memchr ();
extern int memcmp ();
extern int memcpy ();
extern int memcpy_s ();
extern int memimem ();
extern int memmem ();
extern int memmove ();
extern int memmove_s ();
extern int memset ();
extern int memset_s ();
extern int mkstemp ();
extern int mktemp ();
extern int mktime ();
extern int modf ();
extern int modff ();
extern int modfl ();
extern int mrand48 ();
extern int msgQEvStart ();
extern int mtx_destroy ();
extern int mtx_init ();
extern int mtx_lock ();
extern int mtx_timedlock ();
extern int mtx_trylock ();
extern int mtx_unlock ();
extern int nan ();
extern int nanf ();
extern int nanl ();
extern int nanosleep ();
extern int nearbyint ();
extern int nearbyintf ();
extern int nearbyintl ();
extern int nextafter ();
extern int nextafterf ();
extern int nextafterl ();
extern int nexttoward ();
extern int nexttowardf ();
extern int nexttowardl ();
extern int nrand48 ();
extern int objShowAll ();
extern int opendir ();
extern int perror ();
extern int pow ();
extern int powf ();
extern int powl ();
extern int printf ();
extern int printf_s ();
extern int putc ();
extern int putc_unlocked ();
extern int putchar ();
extern int putchar_unlocked ();
extern int putenv ();
extern int puts ();
extern int putw ();
extern int putwc ();
extern int putwchar ();
extern int qsort ();
extern int qsort_s ();
extern int raise ();
extern int rand ();
extern int randBytes ();
extern int rand_r ();
extern int readlink ();
extern int realloc ();
extern int remainder ();
extern int remainderf ();
extern int remainderl ();
extern int remove ();
extern int remquo ();
extern int remquof ();
extern int remquol ();
extern int rename ();
extern int rewind ();
extern int riemann_zeta ();
extern int riemann_zetaf ();
extern int riemann_zetal ();
extern int rindex ();
extern int rint ();
extern int rintf ();
extern int rintl ();
extern int rngCreate ();
extern int round ();
extern int roundf ();
extern int roundl ();
extern int rtcRegister ();
extern int scalbln ();
extern int scalblnf ();
extern int scalblnl ();
extern int scalbn ();
extern int scalbnf ();
extern int scalbnl ();
extern int scanf ();
extern int scanf_s ();
extern int seed48 ();
extern int semEvStart ();
extern int semExchange ();
extern int semInfo ();
extern int setbuf ();
extern int setlocale ();
extern int setvbuf ();
extern int signal ();
extern int sin ();
extern int sinf ();
extern int sinh ();
extern int sinhf ();
extern int sinhl ();
extern int sinl ();
extern int sllInit ();
extern int snprintf ();
extern int snprintf_s ();
extern int snwprintf_s ();
extern int sph_bessel ();
extern int sph_besself ();
extern int sph_bessell ();
extern int sph_legendre ();
extern int sph_legendref ();
extern int sph_legendrel ();
extern int sph_neumann ();
extern int sph_neumannf ();
extern int sph_neumannl ();
extern int spinLockIsrNdInit ();
extern int spinLockIsrNdTake ();
extern int sprintf ();
extern int sprintf_s ();
extern int sqrt ();
extern int sqrtf ();
extern int sqrtl ();
extern int srand ();
extern int srand48 ();
extern int sscanf ();
extern int sscanf_s ();
extern int strcasecmp ();
extern int strcat ();
extern int strcat_s ();
extern int strchr ();
extern int strcmp ();
extern int strcoll ();
extern int strcpy ();
extern int strcpy_s ();
extern int strcspn ();
extern int strdup ();
extern int strerror ();
extern int strerror_s ();
extern int strerrorlen_s ();
extern int strftime ();
extern int strlcat ();
extern int strlcpy ();
extern int strlen ();
extern int strncasecmp ();
extern int strncat ();
extern int strncat_s ();
extern int strncmp ();
extern int strncpy ();
extern int strncpy_s ();
extern int strnlen_s ();
extern int strpbrk ();
extern int strrchr ();
extern int strspn ();
extern int strstr ();
extern int strtod ();
extern int strtof ();
extern int strtoimax ();
extern int strtok ();
extern int strtok_r ();
extern int strtok_s ();
extern int strtol ();
extern int strtold ();
extern int strtoll ();
extern int strtoul ();
extern int strtoull ();
extern int strtoumax ();
extern int strxfrm ();
extern int swprintf ();
extern int swprintf_s ();
extern int swscanf ();
extern int swscanf_s ();
extern int system ();
extern int tan ();
extern int tanf ();
extern int tanh ();
extern int tanhf ();
extern int tanhl ();
extern int tanl ();
extern int taskIdListGet ();
extern int taskOptionsGet ();
extern int taskRestart ();
extern int taskStatusString ();
extern int taskSwapHookAdd ();
extern int taskSwitchHookAdd ();
extern int tempnam ();
extern int tgamma ();
extern int tgammaf ();
extern int tgammal ();
extern int thrd_create ();
extern int thrd_current ();
extern int thrd_detach ();
extern int thrd_equal ();
extern int thrd_exit ();
extern int thrd_join ();
extern int thrd_sleep ();
extern int thrd_yield ();
extern int tiAm3SioDrv ();
extern int tiApllClkDrv ();
extern int tiClkDomainDrv ();
extern int tiClkTreeDrv ();
extern int tiDividerClkDrv ();
extern int tiDpllClkDrv ();
extern int tiDpllX2ClkDrv ();
extern int tiFixedClkDrv ();
extern int tiFixedFactorClkDrv ();
extern int tiGateClkDrv ();
extern int tiModGateClkDrv ();
extern int tiMuxClkDrv ();
extern int time ();
extern int tipStart ();
extern int tmpfile ();
extern int tmpfile_s ();
extern int tmpnam ();
extern int tmpnam_s ();
extern int tolower ();
extern int toupper ();
extern int towctrans ();
extern int towlower ();
extern int towupper ();
extern int trunc ();
extern int truncf ();
extern int truncl ();
extern int tss_create ();
extern int tss_delete ();
extern int tss_get ();
extern int tss_set ();
extern int tzset ();
extern int ungetc ();
extern int ungetwc ();
extern int unldByModuleId ();
extern int unw_getcontext ();
extern int usbXhcdInit ();
extern int valloc ();
extern int vfpArchInit ();
extern int vfprintf ();
extern int vfprintf_s ();
extern int vfscanf ();
extern int vfscanf_s ();
extern int vfwprintf ();
extern int vfwprintf_s ();
extern int vfwscanf ();
extern int vfwscanf_s ();
extern int vprintf ();
extern int vprintf_s ();
extern int vscanf ();
extern int vscanf_s ();
extern int vsnprintf ();
extern int vsnprintf_s ();
extern int vsnwprintf_s ();
extern int vsprintf ();
extern int vsprintf_s ();
extern int vsscanf ();
extern int vsscanf_s ();
extern int vswprintf ();
extern int vswprintf_s ();
extern int vswscanf ();
extern int vswscanf_s ();
extern int vwprintf ();
extern int vwprintf_s ();
extern int vwscanf ();
extern int vwscanf_s ();
extern int vxbAm3PadsDrv ();
extern int vxbAm3RtcDrv ();
extern int vxbFdtArmGenIntCtlrDrv ();
extern int vxbFdtBusDrv ();
extern int vxbFdtDwc3TiDrv ();
extern int vxbFdtI2cPcfRtcDrv ();
extern int vxbFdtOmapGpioDrv ();
extern int vxbFdtTiEdma3Drv ();
extern int vxbFdtUsbDwc3Drv ();
extern int vxbFdtUsbPhyTiAm572xDrv ();
extern int vxbMmcStorageDrv ();
extern int vxbOfAm335xSpiDrv ();
extern int vxbOfAm38xxI2cCtlrDrv ();
extern int vxbOfAm572xSpiDrv ();
extern int vxbOfI2cEepromDrv ();
extern int vxbOfOmap3TimerDrv ();
extern int vxbRootDrv ();
extern int vxbSdStorageDrv ();
extern int vxbSimpleBusDrv ();
extern int vxbSpiDevXfer ();
extern int vxbTiMmchsCtrlDrv ();
extern int vxbUsbCompDevDrv ();
extern int vxbUsbRootHubDrv ();
extern int wcrtomb ();
extern int wcrtomb_s ();
extern int wcscat ();
extern int wcscat_s ();
extern int wcschr ();
extern int wcscmp ();
extern int wcscoll ();
extern int wcscpy ();
extern int wcscpy_s ();
extern int wcscspn ();
extern int wcsftime ();
extern int wcslen ();
extern int wcsncat ();
extern int wcsncat_s ();
extern int wcsncmp ();
extern int wcsncpy ();
extern int wcsncpy_s ();
extern int wcsnlen_s ();
extern int wcspbrk ();
extern int wcsrchr ();
extern int wcsrtombs ();
extern int wcsrtombs_s ();
extern int wcsspn ();
extern int wcsstr ();
extern int wcstod ();
extern int wcstof ();
extern int wcstoimax ();
extern int wcstok ();
extern int wcstok_s ();
extern int wcstol ();
extern int wcstold ();
extern int wcstoll ();
extern int wcstombs ();
extern int wcstombs_s ();
extern int wcstoul ();
extern int wcstoull ();
extern int wcstoumax ();
extern int wcsxfrm ();
extern int wctob ();
extern int wctomb ();
extern int wctomb_s ();
extern int wctrans ();
extern int wctype ();
extern int wmemchr ();
extern int wmemcmp ();
extern int wmemcpy ();
extern int wmemcpy_s ();
extern int wmemmove ();
extern int wmemmove_s ();
extern int wmemset ();
extern int wprintf ();
extern int wprintf_s ();
extern int wscanf ();
extern int wscanf_s ();

FUNC linkSyms [] = {
    _Assert,
    _Atexit,
    _Besseli,
    _Besseli0,
    _Besseli1,
    _Besseli_AS,
    _Besseli_UAE,
    _Besselj,
    _Besselj0,
    _Besselj1,
    _Besselk,
    _Besselk0,
    _Besselk1,
    _Bessely0,
    _Bessely1,
    _Cnd_do_broadcast_at_thread_exit,
    _Dbl,
    _Dclass,
    _Deletegloballocale,
    _Dsign,
    _Exit,
    _FBesseli0,
    _FBesseli1,
    _FBesseli_AS,
    _FBesseli_UAE,
    _FBesselj0,
    _FBesselj1,
    _FBesselk0,
    _FBesselk1,
    _FBessely0,
    _FBessely1,
    _FDclass,
    _FDsign,
    _FFpcomp,
    _FRf_xp,
    _FRiemann_approx,
    _Fenv0,
    _Flt,
    _Fltrounds,
    _Fpcomp,
    _Getctyptab,
    _Getgloballocale,
    _LBesseli0,
    _LBesseli1,
    _LBesseli_AS,
    _LBesseli_UAE,
    _LBesselj0,
    _LBesselj1,
    _LBesselk0,
    _LBesselk1,
    _LBessely0,
    _LBessely1,
    _LDclass,
    _LDsign,
    _LFpcomp,
    _LRf_xp,
    _LRiemann_approx,
    _Ldbl,
    _Lock_shared_ptr_spin_lock,
    _Rf_xp,
    _Riemann_approx,
    _Stderr,
    _Stdin,
    _Stdout,
    _Xmutex_cpp_unused,
    _Xtime_diff_to_ts,
    _ZN6Dinkum7codecvt10_Cvt_checkEmj,
    _ZNSt10filesystem10_Close_dirEPv,
    _ZNSt6_WinitC1Ev,
    _ZNSt6locale6globalERKS_,
    _ZNSt6locale7_Locimp9_MakewlocERKSt8_LocinfoiPS0_PKS_,
    _ZNSt6locale7_Locimp9_MakexlocERKSt8_LocinfoiPS0_PKS_,
    _ZNSt8ios_base4InitC1Ev,
    _ZNSt8ios_base7_AddstdEPS_,
    _ZNSt9exceptionD0Ev,
    _ZSt10_Rng_abortPKc,
    _ZSt11_Xbad_allocv,
    _ZSt11setiosflagsNSt5_IosbIiE9_FmtflagsE,
    _ZSt13_Execute_onceRSt9once_flagPFiPvS1_PS1_ES1_,
    _ZSt13_Syserror_mapi,
    _ZSt14_Debug_messagePKcS0_j,
    _ZSt14_Raise_handler,
    _ZSt14_Random_devicev,
    _ZSt14_Throw_C_errori,
    _ZSt14set_unexpectedPFvvE,
    _ZSt18uncaught_exceptionv,
    _ZSt7_FiopenPKcNSt5_IosbIiE9_OpenmodeEi,
    _ZSt7_MP_AddPyy,
    _ZSt7nothrow,
    _ZSt8_XLgammad,
    _ZSt9terminatev,
    _ZdaPv,
    _ZdaPvRKSt9nothrow_t,
    _ZdaPvS_,
    _ZdaPvj,
    _ZdlPv,
    _ZdlPvRKSt9nothrow_t,
    _ZdlPvS_,
    _ZdlPvj,
    _Znaj,
    _ZnajPv,
    _ZnajRKSt9nothrow_t,
    _Znwj,
    _ZnwjPv,
    _ZnwjRKSt9nothrow_t,
    __absvdi2,
    __absvsi2,
    __adddf3,
    __addsf3,
    __addvdi3,
    __addvsi3,
    __aeabi_cdcmpeq,
    __aeabi_cdcmpeq_check_nan,
    __aeabi_cdcmple,
    __aeabi_cdrcmple,
    __aeabi_cfcmpeq,
    __aeabi_cfcmpeq_check_nan,
    __aeabi_cfcmple,
    __aeabi_cfrcmple,
    __aeabi_d2f,
    __aeabi_d2h,
    __aeabi_d2iz,
    __aeabi_d2lz,
    __aeabi_d2uiz,
    __aeabi_d2ulz,
    __aeabi_dadd,
    __aeabi_dcmpeq,
    __aeabi_dcmpge,
    __aeabi_dcmpgt,
    __aeabi_dcmple,
    __aeabi_dcmplt,
    __aeabi_dcmpun,
    __aeabi_ddiv,
    __aeabi_dmul,
    __aeabi_dneg,
    __aeabi_dsub,
    __aeabi_f2d,
    __aeabi_f2h,
    __aeabi_f2iz,
    __aeabi_f2lz,
    __aeabi_f2uiz,
    __aeabi_f2ulz,
    __aeabi_fadd,
    __aeabi_fcmpeq,
    __aeabi_fcmpge,
    __aeabi_fcmpgt,
    __aeabi_fcmple,
    __aeabi_fcmplt,
    __aeabi_fcmpun,
    __aeabi_fdiv,
    __aeabi_fmul,
    __aeabi_fneg,
    __aeabi_fsub,
    __aeabi_h2f,
    __aeabi_i2d,
    __aeabi_i2f,
    __aeabi_idiv,
    __aeabi_idivmod,
    __aeabi_l2d,
    __aeabi_l2f,
    __aeabi_lasr,
    __aeabi_lcmp,
    __aeabi_ldivmod,
    __aeabi_llsl,
    __aeabi_llsr,
    __aeabi_lmul,
    __aeabi_memclr,
    __aeabi_memclr4,
    __aeabi_memclr8,
    __aeabi_memcmp,
    __aeabi_memcmp4,
    __aeabi_memcmp8,
    __aeabi_memcpy,
    __aeabi_memcpy4,
    __aeabi_memcpy8,
    __aeabi_memmove,
    __aeabi_memmove4,
    __aeabi_memmove8,
    __aeabi_memset,
    __aeabi_memset4,
    __aeabi_memset8,
    __aeabi_ui2d,
    __aeabi_ui2f,
    __aeabi_uidiv,
    __aeabi_uidivmod,
    __aeabi_ul2d,
    __aeabi_ul2f,
    __aeabi_ulcmp,
    __aeabi_uldivmod,
    __aeabi_unwind_cpp_pr0,
    __ashldi3,
    __ashrdi3,
    __clzdi2,
    __clzsi2,
    __cmpdf2,
    __cmpdi2,
    __cmpsf2,
    __ctzdi2,
    __ctzsi2,
    __cxa_bad_cast,
    __cxa_call_unexpected,
    __cxa_demangle,
    __cxa_get_globals,
    __cxa_guard_abort,
    __cxa_pure_virtual,
    __cxa_thread_atexit,
    __cxa_uncaught_exception,
    __cxa_vec_new,
    __divdc3,
    __divdf3,
    __divdi3,
    __divmoddi4,
    __divmodsi4,
    __divsc3,
    __divsf3,
    __divsi3,
    __divtc3,
    __divxc3,
    __dynamic_cast,
    __eprintf,
    __eqdf2,
    __eqsf2,
    __errno,
    __extendhfsf2,
    __extendsfdf2,
    __ffsdi2,
    __fixdfdi,
    __fixdfsi,
    __fixsfdi,
    __fixsfsi,
    __fixunsdfdi,
    __fixunsdfsi,
    __fixunssfdi,
    __fixunssfsi,
    __fixunsxfdi,
    __fixunsxfsi,
    __fixxfdi,
    __floatdidf,
    __floatdisf,
    __floatdixf,
    __floatsidf,
    __floatsisf,
    __floatundidf,
    __floatundisf,
    __floatundixf,
    __floatunsidf,
    __floatunsisf,
    __gedf2,
    __gesf2,
    __gnu_f2h_ieee,
    __gnu_h2f_ieee,
    __gtdf2,
    __gtsf2,
    __ledf2,
    __lesf2,
    __lshrdi3,
    __ltdf2,
    __ltsf2,
    __moddi3,
    __modsi3,
    __muldc3,
    __muldf3,
    __muldi3,
    __mulodi4,
    __mulosi4,
    __mulsc3,
    __mulsf3,
    __multc3,
    __mulvdi3,
    __mulvsi3,
    __mulxc3,
    __nedf2,
    __negdf2,
    __negdi2,
    __negsf2,
    __negvdi2,
    __negvsi2,
    __nesf2,
    __paritydi2,
    __paritysi2,
    __popcountdi2,
    __popcountsi2,
    __powidf2,
    __powisf2,
    __powixf2,
    __subdf3,
    __subsf3,
    __subvdi3,
    __subvsi3,
    __truncdfhf2,
    __truncdfsf2,
    __truncsfhf2,
    __ucmpdi2,
    __udivdi3,
    __udivmoddi4,
    __udivmodsi4,
    __udivsi3,
    __umoddi3,
    __umodsi3,
    __unorddf2,
    __unordsf2,
    __vxatomic,
    __wr_register_frame_info,
    abort,
    abort_message,
    abs,
    absSymbols_Common,
    acos,
    acosf,
    acosh,
    acoshf,
    acoshl,
    acosl,
    aligned_alloc,
    armGenTimerDrv,
    asctime,
    asctime_r,
    asctime_s,
    asin,
    asinf,
    asinh,
    asinhf,
    asinhl,
    asinl,
    assoc_laguerre,
    assoc_laguerref,
    assoc_laguerrel,
    assoc_legendre,
    assoc_legendref,
    assoc_legendrel,
    atan,
    atan2,
    atan2f,
    atan2l,
    atanf,
    atanh,
    atanhf,
    atanhl,
    atanl,
    atexit,
    atof,
    atoi,
    atol,
    atoll,
    atomic_flag_clear,
    atomic_flag_clear_explicit,
    atomic_flag_test_and_set,
    atomic_flag_test_and_set_explicit,
    atomic_signal_fence,
    atomic_thread_fence,
    bcmp,
    bcopy,
    besseli0,
    besseli0f,
    besseli0l,
    besseli1,
    besseli1f,
    besseli1l,
    besselj0,
    besselj0f,
    besselj0l,
    besselj1,
    besselj1f,
    besselj1l,
    besselk0,
    besselk0f,
    besselk0l,
    besselk1,
    besselk1f,
    besselk1l,
    bessely0,
    bessely0f,
    bessely0l,
    bessely1,
    bessely1f,
    bessely1l,
    beta,
    betaf,
    betal,
    bitFirstGet,
    bsearch,
    bsearch_s,
    btowc,
    bzero,
    c16rtomb,
    c32rtomb,
    cabs,
    cabsf,
    cabsl,
    cacos,
    cacosf,
    cacosh,
    cacoshf,
    cacoshl,
    cacosl,
    call_once,
    calloc,
    carg,
    cargf,
    cargl,
    casin,
    casinf,
    casinh,
    casinhf,
    casinhl,
    casinl,
    catan,
    catanf,
    catanh,
    catanhf,
    catanhl,
    catanl,
    cbrt,
    cbrtf,
    cbrtl,
    ccos,
    ccosf,
    ccosh,
    ccoshf,
    ccoshl,
    ccosl,
    ceil,
    ceilf,
    ceill,
    cexp,
    cexpf,
    cexpl,
    cfree,
    checksum,
    cimag,
    cimagf,
    cimagl,
    clearerr,
    clock,
    clog,
    clogf,
    clogl,
    cnd_broadcast,
    cnd_destroy,
    cnd_init,
    cnd_signal,
    cnd_timedwait,
    cnd_wait,
    comp_ellint_1,
    comp_ellint_1f,
    comp_ellint_1l,
    comp_ellint_2,
    comp_ellint_2f,
    comp_ellint_2l,
    comp_ellint_3,
    comp_ellint_3f,
    comp_ellint_3l,
    conj,
    conjf,
    conjl,
    copysign,
    copysignf,
    copysignl,
    cos,
    cosf,
    cosh,
    coshf,
    coshl,
    cosl,
    cpow,
    cpowf,
    cpowl,
    cproj,
    cprojf,
    cprojl,
    cpswPortDrv,
    cpswSwCtrlDrv,
    creal,
    crealf,
    creall,
    csin,
    csinf,
    csinh,
    csinhf,
    csinhl,
    csinl,
    csqrt,
    csqrtf,
    csqrtl,
    ctan,
    ctanf,
    ctanh,
    ctanhf,
    ctanhl,
    ctanl,
    ctime,
    ctime_r,
    ctime_s,
    cyl_bessel_if,
    cyl_bessel_il,
    cyl_bessel_jf,
    cyl_bessel_jl,
    cyl_bessel_kf,
    cyl_bessel_kl,
    cyl_neumann,
    cyl_neumannf,
    cyl_neumannl,
    difftime,
    div,
    dllCreate,
    dllInit,
    dosFsCacheCreate,
    drand48,
    ellint_1,
    ellint_1f,
    ellint_1l,
    ellint_2,
    ellint_2f,
    ellint_2l,
    ellint_3,
    ellint_3f,
    ellint_3l,
    erand48,
    erf,
    erfc,
    erfcf,
    erfcl,
    erff,
    erfl,
    etherMultiAdd,
    exit,
    exp,
    exp2,
    exp2f,
    exp2l,
    expf,
    expint,
    expintf,
    expintl,
    expl,
    expm1,
    expm1f,
    expm1l,
    fabs,
    fabsf,
    fabsl,
    fclose,
    fdim,
    fdimf,
    fdiml,
    fdopen,
    fdtMicrelPhyDrv,
    feclearexcept,
    fegetenv,
    fegetexceptflag,
    fegetround,
    feholdexcept,
    feof,
    feraiseexcept,
    ferror,
    fesetenv,
    fesetexceptflag,
    fesetround,
    fetestexcept,
    feupdateenv,
    fflush,
    fgetc,
    fgetpos,
    fgets,
    fgetwc,
    fgetws,
    fileno,
    flockfile,
    floor,
    floorf,
    floorl,
    fma,
    fmaf,
    fmal,
    fmax,
    fmaxf,
    fmaxl,
    fmin,
    fminf,
    fminl,
    fmod,
    fmodf,
    fmodl,
    fopen,
    fopen_s,
    fppProbe,
    fprintf,
    fprintf_s,
    fputc,
    fputs,
    fputwc,
    fputws,
    fread,
    free,
    freopen,
    freopen_s,
    frexp,
    frexpf,
    frexpl,
    fscanf,
    fscanf_s,
    fseek,
    fseeko,
    fsetpos,
    ftell,
    ftello,
    ftpCommand,
    ftrylockfile,
    funlockfile,
    fwide,
    fwprintf,
    fwprintf_s,
    fwrite,
    fwscanf,
    fwscanf_s,
    getaddrinfo,
    getc,
    getc_unlocked,
    getchar,
    getchar_unlocked,
    getenv,
    getenv_s,
    gethostbyname,
    getifaddrs,
    getnameinfo,
    getopt,
    getoptInit,
    gets,
    gets_s,
    getservbyname,
    getservbyport,
    getw,
    getwc,
    getwchar,
    gmtime,
    gmtime_r,
    gmtime_s,
    hashTblInit,
    hermite,
    hermitef,
    hermitel,
    hookAddToTail,
    hookShow,
    hypot,
    hypotf,
    hypotl,
    ifconfig,
    ilogb,
    ilogbf,
    ilogbl,
    imaxabs,
    imaxdiv,
    index,
    inet_aton,
    inet_network,
    ioHelp,
    ipAttach,
    ipcom_cfgfile_parse,
    ipcom_register_dev_name_mapping,
    ipnet_eth_add_hdr,
    ipnet_ip4_input,
    ipnet_ip4_mcast_input,
    ipnet_ip6_input,
    ipnet_ip6_mcast_input,
    ipnet_sock_netlink_register,
    ipnet_sock_route_register,
    ipnet_vlan_if_init,
    isalnum,
    isalpha,
    isblank,
    iscntrl,
    isdigit,
    isgraph,
    islower,
    isprint,
    ispunct,
    isspace,
    isupper,
    iswalnum,
    iswalpha,
    iswblank,
    iswcntrl,
    iswctype,
    iswdigit,
    iswgraph,
    iswlower,
    iswprint,
    iswpunct,
    iswspace,
    iswupper,
    iswxdigit,
    isxdigit,
    jobAdd,
    jrand48,
    labs,
    laguerre,
    laguerref,
    laguerrel,
    lcong48,
    ldexp,
    ldexpf,
    ldexpl,
    ldiv,
    legendre,
    legendref,
    legendrel,
    lgamma,
    lgammaf,
    lgammal,
    llabs,
    lldiv,
    llrint,
    llrintf,
    llrintl,
    llround,
    llroundf,
    llroundl,
    localeconv,
    localtime,
    localtime_r,
    localtime_s,
    log,
    log10,
    log10f,
    log10l,
    log1p,
    log1pf,
    log1pl,
    log2,
    log2f,
    log2l,
    logb,
    logbf,
    logbl,
    logf,
    logl,
    longjmp,
    lrand48,
    lrint,
    lrintf,
    lrintl,
    lround,
    lroundf,
    lroundl,
    lstInit,
    m_prepend,
    malloc,
    mblen,
    mbrlen,
    mbrtoc16,
    mbrtoc32,
    mbrtowc,
    mbsinit,
    mbsrtowcs,
    mbsrtowcs_s,
    mbstowcs,
    mbstowcs_s,
    mbtowc,
    memalign,
    memchr,
    memcmp,
    memcpy,
    memcpy_s,
    memimem,
    memmem,
    memmove,
    memmove_s,
    memset,
    memset_s,
    mkstemp,
    mktemp,
    mktime,
    modf,
    modff,
    modfl,
    mrand48,
    msgQEvStart,
    mtx_destroy,
    mtx_init,
    mtx_lock,
    mtx_timedlock,
    mtx_trylock,
    mtx_unlock,
    nan,
    nanf,
    nanl,
    nanosleep,
    nearbyint,
    nearbyintf,
    nearbyintl,
    nextafter,
    nextafterf,
    nextafterl,
    nexttoward,
    nexttowardf,
    nexttowardl,
    nrand48,
    objShowAll,
    opendir,
    perror,
    pow,
    powf,
    powl,
    printf,
    printf_s,
    putc,
    putc_unlocked,
    putchar,
    putchar_unlocked,
    putenv,
    puts,
    putw,
    putwc,
    putwchar,
    qsort,
    qsort_s,
    raise,
    rand,
    randBytes,
    rand_r,
    readlink,
    realloc,
    remainder,
    remainderf,
    remainderl,
    remove,
    remquo,
    remquof,
    remquol,
    rename,
    rewind,
    riemann_zeta,
    riemann_zetaf,
    riemann_zetal,
    rindex,
    rint,
    rintf,
    rintl,
    rngCreate,
    round,
    roundf,
    roundl,
    rtcRegister,
    scalbln,
    scalblnf,
    scalblnl,
    scalbn,
    scalbnf,
    scalbnl,
    scanf,
    scanf_s,
    seed48,
    semEvStart,
    semExchange,
    semInfo,
    setbuf,
    setlocale,
    setvbuf,
    signal,
    sin,
    sinf,
    sinh,
    sinhf,
    sinhl,
    sinl,
    sllInit,
    snprintf,
    snprintf_s,
    snwprintf_s,
    sph_bessel,
    sph_besself,
    sph_bessell,
    sph_legendre,
    sph_legendref,
    sph_legendrel,
    sph_neumann,
    sph_neumannf,
    sph_neumannl,
    spinLockIsrNdInit,
    spinLockIsrNdTake,
    sprintf,
    sprintf_s,
    sqrt,
    sqrtf,
    sqrtl,
    srand,
    srand48,
    sscanf,
    sscanf_s,
    strcasecmp,
    strcat,
    strcat_s,
    strchr,
    strcmp,
    strcoll,
    strcpy,
    strcpy_s,
    strcspn,
    strdup,
    strerror,
    strerror_s,
    strerrorlen_s,
    strftime,
    strlcat,
    strlcpy,
    strlen,
    strncasecmp,
    strncat,
    strncat_s,
    strncmp,
    strncpy,
    strncpy_s,
    strnlen_s,
    strpbrk,
    strrchr,
    strspn,
    strstr,
    strtod,
    strtof,
    strtoimax,
    strtok,
    strtok_r,
    strtok_s,
    strtol,
    strtold,
    strtoll,
    strtoul,
    strtoull,
    strtoumax,
    strxfrm,
    swprintf,
    swprintf_s,
    swscanf,
    swscanf_s,
    system,
    tan,
    tanf,
    tanh,
    tanhf,
    tanhl,
    tanl,
    taskIdListGet,
    taskOptionsGet,
    taskRestart,
    taskStatusString,
    taskSwapHookAdd,
    taskSwitchHookAdd,
    tempnam,
    tgamma,
    tgammaf,
    tgammal,
    thrd_create,
    thrd_current,
    thrd_detach,
    thrd_equal,
    thrd_exit,
    thrd_join,
    thrd_sleep,
    thrd_yield,
    tiAm3SioDrv,
    tiApllClkDrv,
    tiClkDomainDrv,
    tiClkTreeDrv,
    tiDividerClkDrv,
    tiDpllClkDrv,
    tiDpllX2ClkDrv,
    tiFixedClkDrv,
    tiFixedFactorClkDrv,
    tiGateClkDrv,
    tiModGateClkDrv,
    tiMuxClkDrv,
    time,
    tipStart,
    tmpfile,
    tmpfile_s,
    tmpnam,
    tmpnam_s,
    tolower,
    toupper,
    towctrans,
    towlower,
    towupper,
    trunc,
    truncf,
    truncl,
    tss_create,
    tss_delete,
    tss_get,
    tss_set,
    tzset,
    ungetc,
    ungetwc,
    unldByModuleId,
    unw_getcontext,
    usbXhcdInit,
    valloc,
    vfpArchInit,
    vfprintf,
    vfprintf_s,
    vfscanf,
    vfscanf_s,
    vfwprintf,
    vfwprintf_s,
    vfwscanf,
    vfwscanf_s,
    vprintf,
    vprintf_s,
    vscanf,
    vscanf_s,
    vsnprintf,
    vsnprintf_s,
    vsnwprintf_s,
    vsprintf,
    vsprintf_s,
    vsscanf,
    vsscanf_s,
    vswprintf,
    vswprintf_s,
    vswscanf,
    vswscanf_s,
    vwprintf,
    vwprintf_s,
    vwscanf,
    vwscanf_s,
    vxbAm3PadsDrv,
    vxbAm3RtcDrv,
    vxbFdtArmGenIntCtlrDrv,
    vxbFdtBusDrv,
    vxbFdtDwc3TiDrv,
    vxbFdtI2cPcfRtcDrv,
    vxbFdtOmapGpioDrv,
    vxbFdtTiEdma3Drv,
    vxbFdtUsbDwc3Drv,
    vxbFdtUsbPhyTiAm572xDrv,
    vxbMmcStorageDrv,
    vxbOfAm335xSpiDrv,
    vxbOfAm38xxI2cCtlrDrv,
    vxbOfAm572xSpiDrv,
    vxbOfI2cEepromDrv,
    vxbOfOmap3TimerDrv,
    vxbRootDrv,
    vxbSdStorageDrv,
    vxbSimpleBusDrv,
    vxbSpiDevXfer,
    vxbTiMmchsCtrlDrv,
    vxbUsbCompDevDrv,
    vxbUsbRootHubDrv,
    wcrtomb,
    wcrtomb_s,
    wcscat,
    wcscat_s,
    wcschr,
    wcscmp,
    wcscoll,
    wcscpy,
    wcscpy_s,
    wcscspn,
    wcsftime,
    wcslen,
    wcsncat,
    wcsncat_s,
    wcsncmp,
    wcsncpy,
    wcsncpy_s,
    wcsnlen_s,
    wcspbrk,
    wcsrchr,
    wcsrtombs,
    wcsrtombs_s,
    wcsspn,
    wcsstr,
    wcstod,
    wcstof,
    wcstoimax,
    wcstok,
    wcstok_s,
    wcstol,
    wcstold,
    wcstoll,
    wcstombs,
    wcstombs_s,
    wcstoul,
    wcstoull,
    wcstoumax,
    wcsxfrm,
    wctob,
    wctomb,
    wctomb_s,
    wctrans,
    wctype,
    wmemchr,
    wmemcmp,
    wmemcpy,
    wmemcpy_s,
    wmemmove,
    wmemmove_s,
    wmemset,
    wprintf,
    wprintf_s,
    wscanf,
    wscanf_s,
    0
};


int * linkDataSyms [] = {
    0
};

