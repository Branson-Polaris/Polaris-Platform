...
MSG_Q_ID 	loggerMsgRecvQId	= MSG_Q_ID_NULL;
...
void loggerTask(void)
{
	...
	for(;;)
	{
		...
		if(msgQReceive(loggerMsgRecvQId, RecvMsgBuffer, sizeof(RecvMsgBuffer), WAIT_FOREVER) != ERROR)
		{
			...
			switch(logMsgBuffer.LogLevel)
			{
				case LOG_ERROR:
					...
					if(logMsgNoWait("%s%s%s", KRED, logMsgBuffer.Buffer, KNRM, 0, 0, 0) == ERROR)
					{
				...
				case LOG_WARNING:
					if(logMsg("%s%s%s", KYEL, logMsgBuffer.Buffer, KNRM, 0, 0, 0) == ERROR)
					{
						logMsg("loggerTask: Error in displaying on console\n",0,0,0,0,0,0);
					}
					break;
...
STATUS loggerInit(void)
{
	...
	{
    	(void) msgQDelete (loggerMsgRecvQId);
