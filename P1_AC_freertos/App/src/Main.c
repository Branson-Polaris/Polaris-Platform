...
#include "../../board_Eval/inc/lpc_phy.h"/* For the PHY monitor support */
#include "inputs.h"
#include "TcpClient.h"
...
static void prvSetupHardware(void)
{
	...
	Board_Init();
	ADCInit();
	TimerInit(LPC_TIMER0);
	TimerSet(LPC_TIMER0,1000,TIMER0_MATCH_REG_NO);
    TimerCallback(&Timer0Callback,TIMER0_MATCH_REG_NO);
...
static void vSetupIFTask(void *pvParameters)
{
	...
	while (1)
	{
		...
		/* Added delay to resolve the messages getting dropped at 1ms communication interval  */
...
#if PRNT_DBG
			DEBUGOUT("\r\nLink connect status: %d:%d", ((physts & PHY_LINK_CONNECTED) != 0),tcp_link);
#endif
		...
		if (!prt_ip)
		{
			if (lpc_netif.ip_addr.addr)
			{
				...
				DEBUGOUT("\r\nIP_ADDR    : %s",ipaddr_ntoa_r((const ip_addr_t* ) &lpc_netif.ip_addr,tmp_buff, 16));
				DEBUGOUT("\r\nNET_MASK   : %s",ipaddr_ntoa_r((const ip_addr_t* ) &lpc_netif.netmask,tmp_buff, 16));
				DEBUGOUT("\r\nGATEWAY_IP : %s",ipaddr_ntoa_r((const ip_addr_t* ) &lpc_netif.gw,tmp_buff, 16));
#endif
				prt_ip = 1;
			}
		}
