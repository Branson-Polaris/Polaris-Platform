...
void UserInterface::ProcessTaskMessage(Message& message)
{
	...
	string details;
	char signal;
	WeldResult UserInterfacetestWelddata;
	switch(message.msgID)
	{	
		case TO_UI_TASK_WELD_RECIPE:
		...
		case LAST_WELD_RESULT_DATA_READ_RES:
			memcpy(&UserInterfacetestWelddata, message.Buffer, sizeof(WeldResult));
			printf("LAST_WELD_RESULT_DATA_READ_RES is recived in UI_T task\n");
			printf("UserInterface.CycleCounter=%d UserInterface.EndHoldForce=%d\n ",UserInterfacetestWelddata.CycleCounter,UserInterfacetestWelddata.EndHoldForce);	
		default:
...
void* UserInterface::UserInterface_Task( void *)
{
	...
	if(NULL != UI)
	{
		...
		while(UI->bIsTaskRunStatus())
		{
			...
			DatareadTestMessage.task_msgQID=CP->GetMsgQId(CommonProperty::cTaskName[UI_T]);
			...
			{
				printf("LAST_WELD_RESULT_DATA_READ_REQ is sent from UI_T task\n");	
			}
			if(msgQReceive(CP->GetMsgQId(CommonProperty::cTaskName[UI_T]),MsgQBuffer,sizeof(MsgQBuffer),WAIT_FOREVER) != ERROR) 
			{
