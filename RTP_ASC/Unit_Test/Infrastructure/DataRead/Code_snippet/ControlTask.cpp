...
void ControlTask::ProcessTaskMessage(Message& message)
{
	...
	switch(message.msgID)
	{
		...
		case TO_CTRL_TASK_SEEK_CMD:
			...
			break;
		case LAST_WELD_RESULT_DATA_READ_RES:
			memcpy(&ctrltestWelddata, message.Buffer, sizeof(WeldResult));
			printf("LAST_WELD_RESULT_DATA_READ_RES is recived in ctrl task\n");
			printf("ctrltestWelddata.CycleCounter=%d ctrltestWelddata.EndHoldForce=%d\n ",ctrltestWelddata.CycleCounter,ctrltestWelddata.EndHoldForce);
		default:
...
{	 
	...
	DataReadWrite_Message DatareadTestMessage;
	...
	if(NULL != ScCtrl)
	{
		...
		{
			if(msgQReceive(CP->GetMsgQId(CommonProperty::cTaskName[CTRL_T]), RecvMsgBuffer, MAX_SIZE_OF_MSG_LENGTH, NO_WAIT) != ERROR)		
			{
			...
			{
				DatareadTestMessage.msgID=LAST_WELD_RESULT_DATA_READ_REQ;
				DatareadTestMessage.task_msgQID=CP->GetMsgQId(CommonProperty::cTaskName[CTRL_T]);
				
				if (io_loop_selector % 400 == 0)
				{
					if(msgQSend(CP->GetMsgQId(CommonProperty::cTaskName[DATA_READ_T]), reinterpret_cast<char*>(&DatareadTestMessage), sizeof(DatareadTestMessage), NO_WAIT, MSG_PRI_NORMAL) == OK)
					{
						printf("LAST_WELD_RESULT_DATA_READ_REQ is sent from ctrl task\n");	
					}
				}
