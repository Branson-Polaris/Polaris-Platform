...
void ControlTask::ProcessTaskMessage(Message& message)
{
	...
	switch(message.msgID)
	{
		...
		case LAST_WELD_RESULT_DATA_WRITE:
			...
			printf("LAST_WELD_RESULT_DATA_WRITE is recived in ctrl task\n");
		default:
...
{	 
	...
	if(NULL != ScCtrl)
	{
		...
		while(ScCtrl->bIsTaskRunStatus())
		{
			...
			{
				DatareadTestMessage.msgID=LAST_WELD_RESULT_DATA_WRITE;
				DatareadTestMessage.task_msgQID=CP->GetMsgQId(CommonProperty::cTaskName[CTRL_T]);
				DatareadTestMessage.buffer[0]=1;
				DatareadTestMessage.buffer[1]=2;
				
				if (io_loop_selector % 400 == 0)
				{
					if(msgQSend(CP->GetMsgQId(CommonProperty::cTaskName[DATA_WRITE_T]), reinterpret_cast<char*>(&DatareadTestMessage), sizeof(DatareadTestMessage), NO_WAIT, MSG_PRI_NORMAL) == OK)
					{
						printf("LAST_WELD_RESULT_DATA_WRITE is sent from ctrl task\n");	
					}
				}
